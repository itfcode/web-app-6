...
using static ITFCodeWA.API.Constants.SwaggerConfiguration.CurrencyController;
...
namespace ITFCodeWA.API.Controllers.References
{
    ...
    [ApiController]
    public class CurrencyController : ReferenceApiController<ICurrencyDataService, CurrencyModel>
    {
        #region Constructors

        public CurrencyController(ILogger<CurrencyController> logger, ICurrentUserService currentUserService, ICurrencyDataService dataService) :
        ...
        [SwaggerResponse(StatusCodes.Status200OK, GET_RESPONSE_DESCRIPTION, typeof(CurrencyModel))]
        ...
        [SwaggerResponse(StatusCodes.Status200OK, POST_RESPONSE_DESCRIPTION, typeof(CurrencyModel))]
        public override async Task<IActionResult> PostAsync([FromBody] CurrencyModel model,
                CancellationToken cancellationToken = default)
            => await base.PostAsync(model, cancellationToken);

        [SwaggerResponse(StatusCodes.Status200OK, PUT_RESPONSE_DESCRIPTION, typeof(CurrencyModel))]
        public override async Task<IActionResult> PutAsync([FromRoute, SwaggerParameter(PUT_ID_PARAMTER_DESCRIPTION)] int id,
                [FromBody] CurrencyModel model, CancellationToken cancellationToken = default)
