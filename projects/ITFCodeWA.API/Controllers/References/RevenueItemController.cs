...
{
    [Route(ApiRoutes.REVENUE_ITEMS)]
    [SwaggerTag(description: TAG_DESCRIPTION)]
    [ApiController]
    public class RevenueItemController : ReferenceApiController<IRevenueItemDataService, RevenueItemModel>
    {
        #region Constructors

        public RevenueItemController(ILogger<RevenueItemController> logger, ICurrentUserService currentUserService, IRevenueItemDataService dataService) :
        ...
        [SwaggerResponse(StatusCodes.Status200OK, GET_RESPONSE_DESCRIPTION, typeof(RevenueItemModel))]
        ...
        [SwaggerResponse(StatusCodes.Status200OK, POST_RESPONSE_DESCRIPTION, typeof(RevenueItemModel))]
        public override async Task<IActionResult> PostAsync([FromBody] RevenueItemModel model,
                CancellationToken cancellationToken = default)
            => await base.PostAsync(model, cancellationToken);

        [SwaggerResponse(StatusCodes.Status200OK, PUT_RESPONSE_DESCRIPTION, typeof(RevenueItemModel))]
        public override async Task<IActionResult> PutAsync([FromRoute, SwaggerParameter(PUT_ID_PARAMTER_DESCRIPTION)] int id,
                [FromBody] RevenueItemModel model, CancellationToken cancellationToken = default)
