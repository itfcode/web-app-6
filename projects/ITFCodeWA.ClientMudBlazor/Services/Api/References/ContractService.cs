using ITFCodeWA.ClientMudBlazor.Services.Api.Base.References;
using ITFCodeWA.ClientMudBlazor.Services.Api.References.Interfaces;
using ITFCodeWA.Models.References;

namespace ITFCodeWA.ClientMudBlazor.Services.Api.References
{
    public class ContractService : ApiReferenceService<ContractModel>, IContractService
    {
        #region Constractor

        public ContractService(ItfHttpClient httpClient) : base(httpClient, ApiEndPoints.References.CONTRACTS)
        {
        }

        #endregion
    }
}